...
router.push(CendanaRoute());
router.replaceAll([CendanaRoute()]);
...
example:
- sl<CendanaController>().increment();
...
@RoutePage()
class CendanaView extends StatefulWidget {
  const CendanaView({super.key});

  @override
  State<CendanaView> createState() => _CendanaViewState();
}

class _CendanaViewState extends State<CendanaView> {
  final controller = sl<CendanaController>();
  CendanaState get state => controller.state;
  ...
  void onReady() {
    ...
    controller.onReady();
    CendanaListener().handle(context);
  }
