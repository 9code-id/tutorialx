...
router.push(AsaksfoakRoute());
router.replaceAll([AsaksfoakRoute()]);
...
example:
- sl<AsaksfoakController>().increment();
...
@RoutePage()
class AsaksfoakView extends StatefulWidget {
  const AsaksfoakView({super.key});

  @override
  State<AsaksfoakView> createState() => _AsaksfoakViewState();
}

class _AsaksfoakViewState extends State<AsaksfoakView> {
  final controller = sl<AsaksfoakController>();
  AsaksfoakState get state => controller.state;
  ...
  void onReady() {
    ...
    controller.onReady();
    AsaksfoakListener().handle(context);
  }
