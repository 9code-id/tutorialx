...
router.push(SeondasafRoute());
router.replaceAll([SeondasafRoute()]);
...
example:
- sl<SeondasafController>().increment();
...
@RoutePage()
class SeondasafView extends StatefulWidget {
  const SeondasafView({super.key});

  @override
  State<SeondasafView> createState() => _SeondasafViewState();
}

class _SeondasafViewState extends State<SeondasafView> {
  final controller = sl<SeondasafController>();
  SeondasafState get state => controller.state;
  ...
  void onReady() {
    ...
    controller.onReady();
    SeondasafListener().handle(context);
  }
